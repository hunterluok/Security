...
public:
	...
	vector<vector<int> > split_data(vector<vector<int> > data, vector<int> label, int sub_label);
	// 获取  概率.
	vector<float> get_like(vector<vector<int> > data); 
	...
	map<int, vector<float> > fit (vector<vector<int> > data, vector<int> label);
	// int predict_line(vector<int> line, map<int, vector<float> >& result);
	int predict_line(vector<int> line);
	float multiply(vector<int> veca, vector<float> vecb);
...
map<int, vector<float> >  mybaye::fit(vector<vector<int> > data, vector<int> label)
{
	...
	for(int i=0; i < keys.size(); ++i)
	{
		...
		vector<vector<int> > temp_subdata;
...
int mybaye::predict_line(vector<int> line) //, map<int, vector<float> >& result);
{
	int line_pred = -1;
	float maxsums = -10000;
...
float mybaye::multiply(vector<int> veca, vector<float> vecb)
{
...
vector<int> mybaye::predict(vector<vector<int> > testdata)
{
	vector<int> pred;
	for(int i = 0; i < testdata.size(); ++i)
	{
		vector<int> temp_line = testdata[i];
...
vector<vector<int> > mybaye::split_data(vector<vector<int> > data, vector<int> label, int sub_label)
{
...
vector<float> mybaye::get_like(vector<vector<int> > data)
{
	int rows = data.size();
