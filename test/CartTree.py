...
class mycarttree:
    ...
    def find_best_split(self, data, label):
        ...
        for feature_index in range(n):
            ...
            for feature_value in feature_values:
                ...
                if temp_ginigain > max_ginigain and len(left_data) >= self.min_leafs and len(right_data) >= self.min_leafs:
    ...
    def fit_old(self, data, label):
        if len(data) == 0:
            return mytree()
        elif len(np.unique(label)) == 1:
            #mytree.label_class = self.get_result(label)
        ...
        if best_feature:
            right = self.fit_old(best_right_data, best_right_label)
            left = self.fit_old(best_left_data, best_left_label)
            return mytree(feature=best_feature, value=best_feature_value, left_tree=left, right_tree=right)
        else:
            return mytree(label_class=self.get_result(label))


    def fit_another(self, data, label):
        another_node = mytree()
        ...
        right = self.fit_another(best_right_data, best_right_label)
        left = self.fit_another(best_left_data, best_left_label)
...
def myjob(q,q_acc, r, data, mylabel):
    ...
    result = mymodel.fit_another(my_data, my_label)
...
if __name__ == "__main__":
    ...
    mymodel = mycarttree(2)
    result = mymodel.fit_another(data, mylabel)
    preds = mymodel.predict(result, data)
