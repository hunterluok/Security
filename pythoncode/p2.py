...
def get_profit(a, k=2):
    ...
    while len(maxs) != k:
        ...
        if least_index == 0:
            ...
            if temp_diff_befor + diff[least_index + 1] > temp_diff_after + diff[least_index - 1] and temp_diff_befor > \
                    diff[least_index - 1]:
                maxs[least_index - 1] = maxs[least_index]
            elif temp_diff_after + diff[least_index - 1] > temp_diff_befor + diff[least_index + 1] and temp_diff_after > \
                    diff[least_index + 1]:
                mins[least_index + 1] = mins[least_index]
            else:
                pass
